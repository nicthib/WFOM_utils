    ...
    for i_run = 1:numel(stimlist{d}) 
        ...
        if ~strcmp(run, 'runA') % do every run except for runA  
            ...
            try
                ...
                bp = [0.02, 0.04, 0.06, 0.08, 0.12, 0.14, 0.16, 0.18, 0.22, 0.24, 0.26, 0.28];
                for sf = 2:length(bp)
                    save_root = ['/local_mount/space/dingus/1/RS_analysis/jrgeco_filtered/',num2str(bp(sf))];
                    disp(['save root =' num2str(bp(sf))])
...
%                     close(vidfile)
                    ...
                    save(fullfile(save_path,stimlist{d}{i_run}{bp(sf)}),'C','S','expl','m','mask');
                    disp(['done saving ' stimlist{d}{i_run}{sf}])
