...
function [m,data,H,Hstd] = LoadData_RS(varargin)
% VERSION HISTORY
%
...
% Spectra check raw LEDs
disp('Checking spectrum of raw LEDS...')
for i = 1:numel(m.LEDs)
    varname = [m.LEDs{i} '_prefilt'];
    [H.(varname), ~] = getHfromKmeans(data.(m.LEDs{i}),m.IDX,0);
    for j = 1:size(H.(varname),1)
        [~,m.spectra_f,m.spectra.([m.LEDs{i} '_prefilt'])(j,:),~] = fastFourTrans(H.(varname)(j,:),m.framerate/m.nLEDs,2);
        m.spectra.([m.LEDs{i} '_prefilt'])(j,:) = medfilt1(m.spectra.([m.LEDs{i} '_prefilt'])(j,:));
    end
    m.spectra.([m.LEDs{i} '_prefilt']) = nanmean(m.spectra.([m.LEDs{i} '_prefilt']));
end
%
...
if m.dofilter
    disp('Filtering LEDs...')
    m.delay = mean(grpdelay(m.Hd));
    ss = size(data.(m.LEDs{1}));
    for i = 1:m.nLEDs
        data.(m.LEDs{i}) = reshape(data.(m.LEDs{i}),[prod(ss(1:2)) ss(3)]);
        data.(m.LEDs{i}) = padarray(data.(m.LEDs{i}),[0 m.delay],'post');
        data.(m.LEDs{i}) = filter(m.Hd,data.(m.LEDs{i}),2);
        data.(m.LEDs{i}) = data.(m.LEDs{i})(:,m.delay+1:end);
        data.(m.LEDs{i})(m.IDX(:) < 1,:) = NaN;
        data.(m.LEDs{i}) = reshape(data.(m.LEDs{i}),ss);
    end
end

% aggressive 3hz LP filter for hemodynamics
% data.green = reshape(lowpass(reshape(data.green,...
%     [size(data.green,1)*size(data.green,2) size(data.green,3)])'...
%     ,5,m.framerate/m.nLEDs)',...
%     [size(data.green,1) size(data.green,2) size(data.green,3)]);
% data.red = reshape(lowpass(reshape(data.red,...
%     [size(data.red,1)*size(data.red,2) size(data.red,3)])'...
%     ,5,m.framerate/m.nLEDs)',...
%     [size(data.red,1) size(data.red,2) size(data.red,3)]);
% %
...
[data.chbo,data.chbr,~] = convert_Hb_new(data.green,data.red,'g','r',m.baseline,m.spectra_file);
%

% jRGECO conversion
disp('Correcting jRGECO...')
[data,m] = jrgeco_correction(data,m);
disp(['jrgeco conversion score: ' mat2str(m.jrgeco_conversion_score) '% (lower is better)'])
%
...
fn = fieldnames(data);
for i = 1:numel(fn)
    [H.(fn{i}), Hstd.(fn{i})] = getHfromKmeans(data.(fn{i}),m.IDX,0);
    for j = 1:size(H.(fn{i}),1)
        [~,m.spectra_f,m.spectra.([fn{i} '_postfilt'])(j,:),~] = fastFourTrans(H.(fn{i})(j,:),m.framerate/m.nLEDs,2);
        m.spectra.([fn{i} '_postfilt'])(j,:) = medfilt1(m.spectra.([fn{i} '_postfilt'])(j,:));
    end
    m.spectra.([fn{i} '_postfilt']) = mean(m.spectra.([fn{i} '_postfilt']));
end
%

% Keep one raw TC from lime
m.TCs.lime_prefilt = H.lime_prefilt(1,:);
m.TCs.green_prefilt = H.green_prefilt(1,:);
m.TCs.red_prefilt = H.red_prefilt(1,:);
m.TCs.lime_postfilt = H.lime(1,:);
m.TCs.green_postfilt = H.green(1,:);
m.TCs.red_postfilt = H.red(1,:);
