...
% % statistics
% runningdurationmin = min(runningBlocksTableSorted.duration) * 0.05; % second
% runningdurationmax = max(runningBlocksTableSorted.duration) * 0.05;
% edges = 0:100:1200;
% histogram(runningBlocksTableSorted.duration,'BinEdges',edges)
% xlabel('Running Duration', 'FontSize', 14, 'FontWeight', 'bold')
% ylabel('Counts', 'FontSize', 14, 'FontWeight', 'bold')
% title('Mouse cm128', 'FontSize', 16)
...
for i = 1: height(runningBlocksTableSorted)
    ...
    if length(rotf) < maxLength
        rotf = [rotf;repelem(0,maxLength - length(rotf))'];
    end
...
%% extract all states
saveDIR = '/home/bahar/Documents/Scripts/State-to-Behavior/Bahar/2021-01-16/figures/onset/cm128/';
...
for state = 1:numstates
    ...
    for i = 1: height(runningBlocksTableSorted)
        ...
        if length(prob) < maxLength
            prob = [prob;repelem(0,maxLength - length(prob))'];
        end
...
for i = 1: height(runningBlocksTableSorted)
    ...
    if length(pupil) < maxLength
        pupil = [pupil;repelem(pupil(1),maxLength - length(pupil))'];
        %pupil = [pupil;repelem(0,maxLength - length(pupil))'];
    end
...
for i = 1: height(runningBlocksTableSorted)
    ...
    if length(whisk) < maxLength
        whisk = [whisk;repelem(whisk(1),maxLength - length(whisk))'];
        %whisk = [whisk;repelem(0,maxLength - length(whisk))'];
    end
